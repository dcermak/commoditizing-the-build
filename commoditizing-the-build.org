...
#+begin_notes
- parser works via recursive text expansion, sadly no AST at any point
- macros mostly used for vars, conditionals, 
#+end_notes
...
- "functions": =%pytest=
- sections: =%prep=, =%build=
- declarative build system & specparts

** Filelists

#+begin_notes
- rpm packages must declare what lands in the package \Rightarrow =%files=
- can be:
  ordinary file/dir
   =%config=: replaceable config file
   =%config(noreplace)=: config file that is not replaced on updates
   =%doc=, =%license=: get put into special locations
#+end_notes

#+ATTR_REVEAL: :frag (appear) :code_attribs data-line-numbers='|1|2|3-4|5-6|'
#+begin_src spec
%files -f my-list.txt
%{_bindir}/%{name}
...
- but the trade offs are big, especially if we loose zchunk compression, we loose a lot and gain too little
#+end_notes
